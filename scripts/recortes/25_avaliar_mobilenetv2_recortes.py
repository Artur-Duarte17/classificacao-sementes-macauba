...
# ============================================================
# SCRIPT 25 - AVALIAR MOBILENETV2 COM RECORTES
# ------------------------------------------------------------
# Objetivo:
# - Avaliar a MobileNetV2 treinada nos recortes
# - Escolher thresholds somente na validacao
# - Avaliar o teste final com os mesmos cenarios dos outros modelos
#
# Este script nao treina modelo, mas carrega o checkpoint ja treinado.
# ============================================================
...
class DatasetSementes(Dataset):
    ...
    def __getitem__(self, indice):
        ...
        return {
            ...
            "caminho_imagem": str(linha["caminho_imagem"]),
...
def obter_predicoes(modelo, carregador, dispositivo) -> pd.DataFrame:
    ...
    for lote in carregador:
        imagens = transferir_imagens(lote["imagem"], dispositivo)
        ...
        for indice, prob in enumerate(probabilidades.cpu().tolist()):
            registros.append({
                ...
                "caminho_imagem": lote["caminho_imagem"][indice],
...
def escolher_threshold_por_recall(df_thresholds: pd.DataFrame) -> float:
    ...
    if candidatos.empty:
        ...
        ordenacao = ["f1_contaminada", "recall_contaminada", "especificidade_nao_contaminada"]

    melhor = candidatos.sort_values(ordenacao, ascending=[False, False, False]).iloc[0]
    return float(melhor["threshold"])


def adicionar_predicoes_threshold(df: pd.DataFrame, sufixo: str, threshold: float):
    df[f"predito_threshold_{sufixo}"] = np.where(
        df["prob_contaminada"] >= threshold,
        "contaminada",
        "nao_contaminada",
    )
...
def plotar_matriz_confusao(metricas: dict, caminho_saida: Path):
    ...
    ax.set_title("MobileNetV2 recortes - matriz de confusao no teste")
...
def plotar_curva_threshold(df_thresholds: pd.DataFrame, caminho_saida: Path):
    ...
    ax.set_title("MobileNetV2 recortes - metricas por threshold na validacao")
...
def main():
    print("=" * 60)
    print("AVALIANDO MOBILENETV2 COM RECORTES")
    print("=" * 60)
    ...
    if not CAMINHO_MODELO.exists():
        raise FileNotFoundError(
            ...
            "Execute primeiro: python scripts\\recortes\\24_treinar_mobilenetv2_recortes.py"
        )
    ...
    for split in ["validacao", "teste"]:
        ...
        carregador = criar_data_loader(dataset)
    ...
    df_metricas = pd.DataFrame([
        {"modelo": NOME_MODELO, "cenario": cenario, **metricas}
        for cenario, metricas in resultados.items()
    ])
    df_metricas.to_csv(CAMINHO_METRICAS, index=False, encoding="utf-8-sig")

    adicionar_predicoes_threshold(df_teste, "0_50", 0.50)
    adicionar_predicoes_threshold(df_teste, "melhor_f1_validacao", threshold_f1)
